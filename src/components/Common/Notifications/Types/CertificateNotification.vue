...
<script>
import Seal from "vue-material-design-icons/Seal";
...
export default {
  ...
  methods: {
    onNotificationClick() {
      ...
      return this.handleRedirect("iu-certificates-dashboard");
    },
    handleRedirect(routeTo) {
      if (!this.notification.action)
        return this.$emit("push-route", { name: routeTo });
      if (
      ...
      this.$emit("push-route", { routeTo: "homepage" });
    },
